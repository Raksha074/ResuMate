...
const MinimalImageTemplate = ({ data, accentColor }) => {
    ...
    <div className="mb-6 flex justify-center">
        <img 
            src={
                data.personal_info.image.includes("ik.imagekit.io") 
                ? (() => {
                    // 1. Remove # and take only the first 6 characters (ignores the 'ff' if present)
                    const cleanColor = accentColor.replace('#', '').substring(0, 6);
                    
                    // 2. Build the ImageKit URL with a cache buster
                    return `${data.personal_info.image}?tr=e-bgremove,bg-${cleanColor}&v=${cleanColor}`;
                  })()
                : data.personal_info.image
            } 
            alt="Profile" 
            className="w-32 h-32 object-cover rounded-full mx-auto shadow-sm transition-all duration-300"
        />
    </div>
) : (
            /* ... keep your 'object' check for local previews below ... */
